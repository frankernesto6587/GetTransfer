...
export function Sidebar({ open, onClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 bottom-0 w-[260px] bg-surface border-r border-border flex flex-col z-50 transition-transform duration-200 ${
      ...
      >
        ...
        <div className="px-6 py-6 flex items-center gap-3">
          ...
          <span className="font-headline text-lg font-semibold tracking-wide text-white">GETTRANSFER</span>
        </div>

        {/* Navigation */}
        <nav className="flex-1 px-3 mt-4">
          {visibleItems.map(item => {
            const isActive = item.to === '/' ? matchRoute({ to: '/', fuzzy: false }) : matchRoute({ to: item.to, fuzzy: true })
            ...
            return (
              <Link
                ...
                className={`w-full flex items-center gap-3 px-3 py-3 md:py-2.5 rounded-lg mb-1 transition-colors text-sm ${
                  isActive ? 'bg-gold-dim text-gold' : 'text-secondary hover:text-white hover:bg-white/5'
                }`}
              >
                <item.icon size={18} />
            ...
            <div className="flex items-center gap-3">
              {user.picture ? (
                ...
                <div className="w-8 h-8 rounded-full bg-border flex items-center justify-center">
                  <span className="text-xs text-secondary font-medium">{user.name?.[0]?.toUpperCase() || '?'}</span>
                </div>
              )}
