...
export async function getcodeRoutes(app: FastifyInstance) {
  app.addHook('preHandler', requireRole('admin', 'confirmer'));
  app.post('/api/getcode/buscar', async (request, reply) => {
  ...
  app.post('/api/getcode/:id', async (request, reply) => {
  ...
  app.get('/api/getcode/:codigo', async (request, reply) => {
  ...
  app.post('/api/getcode/:codigo/liberar', async (request, reply) => {
    const parsed = codigoSchema.safeParse(request.params);
    if (!parsed.success) {
      return reply.status(400).send({ error: 'Codigo invalido' });
    }
